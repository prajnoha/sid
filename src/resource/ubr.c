...
static char *_do_compose_key(struct sid_buf *buf, struct kv_key_spec *key_spec, int prefix_only)
{
	...
	if (buf) {
		if (sid_buf_add_fmt(buf,
		                       ...
		                       key_spec->dom ?: ID_NULL,
		                       ns_to_key_prefix_map[key_spec->ns],
		                       key_spec->ns_part ?: ID_NULL,
		                       key_spec->id_cat ?: ID_NULL,
		                       key_spec->id ?: ID_NULL,
		                       prefix_only ? KV_KEY_NULL : key_spec->core ?: ID_NULL) < 0)
			key = NULL;
	} else {
...
{
	struct kv_key_spec key_spec =
		KV_KEY_SPEC(.extra_op = archive == 1 ? KV_PREFIX_OP_ARCHIVE_C : KV_PREFIX_OP_BLANK_C,
	                    .op       = KV_OP_SET,
	                    .dom      = dom,
	                    .ns       = ns,
	                    .ns_part  = _get_ns_part(ucmd_ctx, owner, ns),
...
{
	struct kv_key_spec key_spec =
		KV_KEY_SPEC(.extra_op = archive ? KV_PREFIX_OP_ARCHIVE_C : KV_PREFIX_OP_BLANK_C,
	                    .op       = KV_OP_SET,
	                    .dom      = dom,
	                    .ns       = ns,
                              ...
                              const char          *new_alias)
{
	...
	key_spec = KV_KEY_SPEC(.op      = KV_OP_SET,
                                     ...
                                     const char             *group_id)
{
	...
	struct kv_key_spec key_spec     = KV_KEY_SPEC(.op      = KV_OP_SET,
                                                  .dom     = dom ?: ID_NULL,
                                                  .ns      = group_ns,
                                                  .ns_part = _get_ns_part(ucmd_ctx, owner, group_ns),
                                                  .id_cat  = group_cat,
                                      ...
                                      sid_ucmd_kv_flags_t *flags)
{
	...
	struct kv_key_spec key_spec = KV_KEY_SPEC(.op   = KV_OP_SET,
	                                          .dom  = KV_KEY_DOM_USER,
	                                          .ns   = SID_KV_NS_DEVICE,
	                                          /* .ns_part will be calculated later */
	                                          .core = key_core);
...
static int _init_common(sid_res_t *res, const void *kickstart_data, void **data)
{
	...
	mod_reg_res_params = &(struct sid_mod_reg_res_params) {
		...
		.symbol_params = SID_MOD_SYM_PARAMS_ARRAY(
			...
			SID_MOD_SYM_PARAMS(.name = SID_UCMD_MOD_FN_NAME_SCAN_POST_CURRENT, .flags = SID_MOD_SYM_FL_INDIRECT),
			...
			SID_MOD_SYM_PARAMS(.name = SID_UCMD_MOD_FN_NAME_SCAN_ACTION_CURRENT, .flags = SID_MOD_SYM_FL_INDIRECT),
